...
class Solver:
    ...
    def fill_hidden_singles(self) -> bool:
        for cell in self.sudoku:
            for digit, group_type in product(cell.pencil_marks, RCB):
                ...
                if _only_one_cell_in_group_can_contain_digit(digit, group):
                    cell.fill(digit)
                    self.sudoku.update_pencil_marks()
                    return True
        return False
        ...
        for size, group_type in product(range(2, 5), RCB_ITER):
            for group in getattr(self.sudoku, group_type):
                ...
                for candidate_tuple in candidate_tuples:
                    if _cells_form_naked_tuple(*candidate_tuple):
        ...
        for group_type, digit in product(RC_ITER, range(1, 10)):
            for group in getattr(self.sudoku, group_type):
                cells_with_digit = {cell
                                    for cell in group
                                    if digit in cell}
                if _cells_share_a_box(*cells_with_digit):
    ...
    def check_for_pointing_tuple(self) -> bool:
        for digit, box in product(range(1, 10), self.sudoku.boxes):
            ...
            if _remove_digits_from_cells(digit, *pointed):
                return True
        return False
    ...
    def check_for_fish(self) -> bool:
        ...
        for size, group_type, digit in product(sizes, RC, range(1, 10)):
            ...
            fish_groups = _trimmed_fish_groups(candidate_groups, group_type, size)
    ...
    def check_for_ywing(self) -> bool:
        ...
        if ywings := _find_valid_ywings(triples):
            for ywing in ywings:
    ...
    def check_for_avoidable_rectangle(self) -> bool:
        for rectangle in self.potential_avoidable_rectangles():
            if _clear_avoidable_rectangle(*rectangle):
                return True
        return False
    ...
    def check_for_unique_rectangle(self) -> bool:
        for box in self.sudoku.boxes:
            ...
            for a, b in combinations(pairs, r=2):
                if _cells_form_naked_tuple(a, b):
        ...
        for a, b in combinations([cell for cell in self.sudoku if cell.is_empty], r=2):
            ...
            if a.y == b.y:
                for key in a.column:
                    ...
                    digits = _pointing_rectangle_digits(a, b, c, d)
    ...
    def check_for_hidden_rectangle(self) -> bool:
        for rectangle in self.sudoku.rectangles():
            if _cells_are_empty(*rectangle) and _at_least_one_cell_has_only_two_options(*rectangle):
    ...
    def check_for_hidden_rectangle_singles(self, rectangle):
        for check_cell in rectangle:
            ...
            if len(pencil_marks) == 2:
                ...
                if _all_cells_in_group_contain_pencil_marks(group, pencil_marks):
                    opposite = {opp
    ...
    def check_for_hidden_rectangle_pairs(self, rectangle) -> bool:
        for group_type, pair in product(RC, combinations(rectangle, r=2)):
            ...
            if _cells_share_axis(check_axis, *pair) and _cells_form_naked_tuple(*pair):
                pencil_marks = pair[0].pencil_marks
                opposite_pair: set[Cell, Cell] = {*rectangle} - {*pair}
                if _all_cells_in_group_contain_pencil_marks(opposite_pair, pencil_marks) is False:
                    continue
                for digit in pencil_marks:
    ...
    def check_for_skyscraper(self) -> bool:
        for digit in range(1, 10):
            for row_1, row_2 in combinations(self.sudoku.rows, r=2):
                row_1 = {cell for cell in row_1 if digit in cell}
                row_2 = {cell for cell in row_2 if digit in cell}
                pass
    ...
    def clear_locked_candidate(self, locked_candidate, digit) -> bool:
        ...
        if _remove_digits_from_cells(digit, *affected):
            return True
        return False
    ...
    def clear_fish(self, digit, fish_groups, group_type) -> bool:
        ...
        perpendicular_fish_groups = _perpendicular_groups(fish_groups, group_type)
        ...
        for perp_group in perpendicular_fish_groups:
            ...
            if _remove_digits_from_cells(digit, *cells):
                operated = True
        return operated
    ...
    def clear_ywing(self, ywing) -> bool:
        ...
        if _remove_digits_from_cells(shared_digit, *affected_cells):
            return True
        return operated

    def clear_xyzwing(self, index, triple):
        shared_digit = _xyzwing_triple_shared_digit(index, triple)
    ...
    @staticmethod
    def clear_naked_tuples(group, tuple_cells) -> bool:
        ...
        if _remove_digits_from_cells(options, *non_members):
            return True

    @staticmethod
    def clear_hidden_tuple(group, size) -> bool:
        for digits in combinations(range(1, 10), r=size):
            candidate_tuple = _cells_in_group_with_digits(digits, group)
            if len(candidate_tuple) != size: continue
            if _cells_form_hidden_tuple(digits, candidate_tuple):
                other_digits = set(range(1, 10)) - set(digits)
                if _remove_digits_from_cells(other_digits, *candidate_tuple):
                    return True
        return False

    @staticmethod
    def clear_hidden_rectangle_pair(digit, pair, pencil_marks) -> bool:
        operated = False
    ...
    def cells_seen_by_pointing_tuple(self, pointing):
        if _cells_share_a_row(*pointing):
            pointed = set(self.sudoku.row(next(iter(pointing)).y))
        elif _cells_share_a_column(*pointing):
    ...
    def potential_avoidable_rectangles(self) -> Generator[tuple[Cell, Cell, Cell, Cell], None, None]:
        ...
        for rectangle in self.sudoku.rectangles():
            if _rectangle_is_avoidable(rectangle):
                yield rectangle
    ...
    def cells_are_closely_related_by_digit(self, digit: int, *cells: Cell) -> bool:
        ...
        if _cells_share_a_column(*cells):
            axis = "x"
            group_type = "column"
        elif _cells_share_a_row(*cells):
            axis = "y"
            group_type = "row"
        else:
            return False
...
def _all_cells_in_group_contain_pencil_marks(group: Iterable[Cell], pencil_marks: set[int]) -> bool:
...
def _cells_share_axis(check_axis, *cells: Cell) -> bool:
...
def _cells_form_naked_tuple(*cells) -> bool:
...
def _cells_are_empty(*cells) -> bool:
...
def _clear_avoidable_rectangle(top_left, top_right, bot_left, bot_right) -> bool:
    operated = False
...
def _find_valid_ywings(triples) -> list[Optional[list[Cell, Cell]]]:
...
def _xyzwing_triple_shared_digit(index, triple: tuple[Cell, Cell, Cell]) -> Optional[int]:
...
def _rectangle_is_avoidable(rectangle) -> bool:
...
def _trimmed_fish_groups(candidate_groups, group_type, size) -> Optional[tuple]:
...
def _perpendicular_groups(group_list: list[list[Cell]], group_type: str) -> list[list[Cell]]:
...
def _cells_in_group_with_digits(digits, group):
    empty_cells = {cell for cell in group if cell.is_empty}
    cells_with_digits = [
...
def _cells_form_hidden_tuple(digits, candidate_tuple) -> bool:
    digits = set(digits)
...
def _only_one_cell_in_group_can_contain_digit(digit, group) -> bool:
    return len({cell for cell in group if digit in cell}) == 1


def _cells_share_a_box(*cells: Cell) -> bool:
    return len({cell.box_num for cell in cells}) == 1


def _cells_share_a_column(*cells: Cell) -> bool:
    return len({cell.x for cell in cells}) == 1


def _cells_share_a_row(*cells: Cell) -> bool:
    return len({cell.y for cell in cells}) == 1


def _remove_digits_from_cells(digits: Union[int, Iterable[int]], *cells: Cell) -> bool:
    operated = False
    for cell in cells:
