...
class Sudoku:
    ...
    def post_init(self, edited: dict[tuple[int, int], [set[int]]]) -> None:
        ...
        :param edited: e.g. (1, 1): set() or (2, 2): {1, 5, 9}
        """
    ...
    def phistomefel_sets(self, left_col: int = None, right_col: int = None,
        ...
        >>> Sudoku.phistomefel_sets(left_col=2, right_col=6, top_row=2, bot_row=6)
        ...
        if min([isinstance(param, int) for param in (left_col, right_col, top_row, bot_row)]):
            return self.single_phistomefel_set(left_col, right_col, top_row, bot_row)
        ...
        for a, b, c, d in product(
            ...
            sets.append(self.single_phistomefel_set(a, b, c, d))

        return sets

    def single_phistomefel_set(self, left_col: int, right_col: int, top_row: int, bot_row: int) \
